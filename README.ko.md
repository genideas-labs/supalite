...
- **공유 싱글턴에서 수동 트랜잭션이 안전해짐 (#17 후속)** — **breaking**: `begin()`이 클라이언트를 변경하는 대신 **연결 고정 핸들**을 반환합니다(`const tx = await db.begin(); … await tx.commit()`). 모듈 전역 싱글턴에 호출해도 다른 동시 쿼리가 트랜잭션으로 라우팅되지 않습니다. `commit()`/`rollback()`(및 중첩 `begin()`)은 오용 시 throw하며, 트랜잭션 스코프는 메타데이터 캐시를 오너와 공유하지 않습니다. `transaction(cb)`는 그대로입니다. 마이그레이션은 [트랜잭션](#트랜잭션) 참고.

이전(0.12.0): **`supalite migrate mark-applied --dry-run` (#14)** — 채택 전 **write-free** 미리보기(기록될 버전+실행 SQL, 쓰기 없음); `up --dry-run`도 write-free + 파일 경로.

이전(0.11.0): **`supalite migrate` (#7)** — 내장 `up` / `status` / `new` / `mark-applied` 러너(advisory lock, 마이그레이션별 원자적 기록, `transaction:false` 이스케이프)로 `db pull → migrate → gen types` 툴체인 완결; **`db pull --format dbmate` (#8)** 베이스라인을 `-- migrate:up` / `-- migrate:down` 마커로 감쌈.
