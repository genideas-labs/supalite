...
- **`supalite migrate mark-applied --dry-run` (#14)**: 기존 DB 채택 전, 첫 쓰기 전에 미리보기 — 추적 테이블을 만들지 않고 **아무것도 쓰지 않으면서** 기록될 버전과 실제 실행 SQL을 그대로 출력합니다. `up --dry-run`도 이제 write-free이며 미적용 마이그레이션을 파일 경로와 함께 보여줍니다.

이전(0.11.0): **`supalite migrate` (#7)** — 내장 `up` / `status` / `new` / `mark-applied` 러너(advisory lock, 마이그레이션별 원자적 기록, `transaction:false` 이스케이프)로 `db pull → migrate → gen types` 툴체인 완결; **`db pull --format dbmate` (#8)** 베이스라인을 `-- migrate:up` / `-- migrate:down` 마커로 감쌈; `gen types` 케이싱 버그(`splitWords`가 문자 `s`를 삼킴) 수정.
