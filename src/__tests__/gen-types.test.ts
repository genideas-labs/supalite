...
describe('generateTypes', () => {
  ...
  test('renders SETOF-table return with SetofOptions and inline row columns', async () => {
    ...
    expect(output).toContain(
      `SetofOptions: { from: '*'; to: 'gen_types_users'; isOneToOne: false; isSetofReturn: true; };`
    );
  ...
  test('casing options rewrite type, function and composite names', async () => {
    const camel = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supalite',
      typeCase: 'camel',
    ...
    const snake = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supalite',
      typeCase: 'snake',
  ...
  test('bigintType option controls bigint column typing', async () => {
    const asString = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supalite',
      bigintType: 'string',
    });
    const asNumber = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supalite',
      bigintType: 'number',
  ...
  test('includeRelationships:false emits placeholder Relationships', async () => {
    const supalite = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supalite',
      includeRelationships: false,
    });
    const supabase = await generateTypes({
      dbUrl: connectionString,
      schemas: ['public'],
      format: 'supabase',
      includeRelationships: false,
  ...
  test('dumpFunctionsSql honours includeProcedures option', async () => {
    const withProcedures = await dumpFunctionsSql({
      dbUrl: connectionString,
      schemas: ['public'],
      includeProcedures: true,
    });
    const functionsOnly = await dumpFunctionsSql({
      dbUrl: connectionString,
      schemas: ['public'],
