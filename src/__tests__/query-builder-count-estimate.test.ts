...
const getMockQuery = () => (jest.requireMock('pg') as { __mockQuery: jest.Mock }).__mockQuery;

const buildExplainRow = (rows: number) => ({
...
describe('QueryBuilder planned/estimated count', () => {
  ...
  beforeEach(() => {
    const pool = new Pool();
    mockQuery = getMockQuery() ?? (pool.query as jest.Mock);
    mockQuery.mockReset();
