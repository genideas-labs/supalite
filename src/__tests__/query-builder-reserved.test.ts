...
import { DatabaseSchema, TableBase } from '../types'; // Removed unused Json import

config(); // Load .env variables

// Define Row/Insert/Update types for the reserved_keyword_test_table
type ReservedKeywordTableRow = { id: number; order: number | null; desc: string | null; user: string | null; limit: number | null; group: string | null };
type ReservedKeywordTableInsert = { id?: number; order?: number | null; desc?: string | null; user?: string | null; limit?: number | null; group?: string | null };
type ReservedKeywordTableUpdate = { id?: number; order?: number | null; desc?: string | null; user?: string | null; limit?: number | null; group?: string | null };

// Define our test-specific database schema including the new table
interface TestDatabaseWithReserved extends DatabaseSchema {
  public: {
    Tables: {
      reserved_keyword_test_table: TableBase & { Row: ReservedKeywordTableRow; Insert: ReservedKeywordTableInsert; Update: ReservedKeywordTableUpdate; Relationships: [] };
      // Potentially include other tables from query-builder-single.test.ts if needed for complex tests, or keep separate
    };
...
describe('QueryBuilder with Reserved Keyword Columns', () => {
  let client: SupaLitePG<TestDatabaseWithReserved>;
  let pool: Pool;
  ...
  beforeEach(async () => {
    client = new SupaLitePG<TestDatabaseWithReserved>({ connectionString });
  ...
  test('should INSERT data into reserved keyword column names', async () => {
    const insertValues: ReservedKeywordTableInsert = {
      id: 4,
      order: 300,
      desc: 'Description D',
      user: 'user_d',
  ...
  test('should UPDATE data in reserved keyword column names', async () => {
    const updateValues: ReservedKeywordTableUpdate = {
  ...
  test('should UPSERT data with reserved keyword column names and onConflict target', async () => {
    const upsertData: ReservedKeywordTableInsert = {
      id: 1, // Existing ID
      order: 101, 
    ...
    const upsertConflictData: ReservedKeywordTableInsert = {
