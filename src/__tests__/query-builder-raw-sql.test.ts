...
const stringifyJsonWithBigint = (value: unknown) =>
...
describe('QueryBuilder raw SQL assertions', () => {
  ...
  const setColumnTypes = (types: Record<string, string>) => {
    (client as any).getColumnPgType = jest.fn(async (_schema: string, _table: string, column: string) => (
      types[column] || 'text'
    ));
  };
  ...
  it('should build match() conditions in order', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."users" WHERE "status" = $1 AND "role" = $2'
    );
  });
  ...
  it('should build chained filter operators with correct placeholders', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."users" WHERE "id" = $1 AND "status" != $2 AND "age" > $3 AND "score" >= $4 AND "rank" < $5 AND "rank" <= $6 AND "email" LIKE $7 AND "nickname" ILIKE $8 AND "deleted_at" IS $9'
    );
  });

  it('should render IS NULL without a placeholder', async () => {
    const qb = new QueryBuilder(pool, client, 'profiles', 'public')
      .select('*')
      .eq('user_id', 1)
      .is('avatar_url', null);
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."profiles" WHERE "user_id" = $1 AND "avatar_url" IS NULL'
    );
  });
  ...
  it('should render IS NOT NULL for not(is, null)', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."users" WHERE "status" = $1 AND "deleted_at" IS NOT NULL'
    );
  });
  ...
  it('should build or() with eq operators', async () => {
    ...
    expect(values).toEqual(['active', 'admin']);
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."users" WHERE ("status" = $1 OR "role" = $2)'
    );
  });
  ...
  it('should build or() with ilike and neq operators', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."users" WHERE ("email" ILIKE $1 OR "status" != $2)'
    );
  });
  ...
  it('should order placeholders correctly for IN + contains + order + limit/offset', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."posts" WHERE ("user_id" IN ($1,$2) OR "user_id" IS NULL) AND "tags" @> $3 ORDER BY "created_at" DESC LIMIT 10 OFFSET 20'
    );
  });
  ...
  it('should render ORDER BY ASC when no options are provided', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."posts" ORDER BY "created_at" ASC'
    );
  });
  ...
  it('should render NULLS LAST when nullsFirst is false', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."shop_gen_images" ORDER BY "pass_no" ASC NULLS LAST'
    );
  });
  ...
  it('should render NULLS FIRST with chained order clauses', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."shop_gen_images" WHERE "request_hash" = $1 ORDER BY "is_final" ASC, "pass_no" ASC NULLS FIRST, "created_at" ASC'
    );
  });
  ...
  it('should translate range() into LIMIT and OFFSET', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT * FROM "public"."comments" LIMIT 4 OFFSET 2'
    );
  });
  ...
  it('should build count exact subquery', async () => {
    ...
    expect(values).toEqual([]);
    expect(normalizeSql(query)).toBe(
      'SELECT *, COUNT(*) OVER() as exact_count FROM (SELECT * FROM "public"."users") subquery'
    );
  });
  ...
  it('should apply order/limit/offset after exact count subquery', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT *, COUNT(*) OVER() as exact_count FROM (SELECT * FROM "public"."orders" WHERE "menu_id" = $1 AND "table_name" = $2 AND "order_closed_time" IS NULL) subquery ORDER BY "created_at" DESC LIMIT 5 OFFSET 10'
    );
  });
  ...
  it('should build head count query with where clause', async () => {
    ...
    expect(values).toEqual(['active']);
    expect(normalizeSql(query)).toBe(
      'SELECT COUNT(*) FROM "public"."users" WHERE "status" = $1'
    );
  });
  ...
  it('should build INSERT without RETURNING when select() is not called', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."users" ("name","status") VALUES ($1,$2)'
    );
  });
  ...
  it('should build INSERT with RETURNING when select() is called', async () => {
    ...
    expect(values).toEqual(['Alice', 'active']);
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."users" ("name","status") VALUES ($1,$2) RETURNING "id", "name"'
    );
  });
  ...
  it('should emit DEFAULT for undefined fields on multi-row insert', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."users" ("name","status","age") VALUES ($1,$2,DEFAULT),($3,DEFAULT,$4)'
    );
  });

  it('should build INSERT with ignoreDuplicates as DO NOTHING', async () => {
    setColumnTypes({});
    const qb = new QueryBuilder(pool, client, 'menu_section_items', 'public')
      .insert(
        [
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."menu_section_items" ("section_id","menu_item_id") VALUES ($1,$2),($3,$4) ON CONFLICT ("section_id", "menu_item_id") DO NOTHING RETURNING *'
    );
  });
  ...
  it('should build UPSERT with comma-separated conflict target', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."menu_item_opts_schema" ("set_id","name") VALUES ($1,$2) ON CONFLICT ("set_id", "name") DO UPDATE SET "set_id" = EXCLUDED."set_id", "name" = EXCLUDED."name" RETURNING *'
    );
  });
  ...
  it('should omit undefined fields on upsert', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."users" ("id","name") VALUES ($1,$2) ON CONFLICT ("id") DO UPDATE SET "id" = EXCLUDED."id", "name" = EXCLUDED."name" RETURNING *'
    );
  });

  it('should quote reserved columns in onConflict targets', async () => {
    setColumnTypes({});
    const qb = new QueryBuilder(pool, client, 'reserved_keyword_test_table', 'public')
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."reserved_keyword_test_table" ("order","desc") VALUES ($1,$2) ON CONFLICT ("order") DO UPDATE SET "order" = EXCLUDED."order", "desc" = EXCLUDED."desc" RETURNING *'
    );
  });
  ...
  it('should build UPSERT with array conflict target', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."ext_menu_item_section_change" ("ext_menu_id","ext_menu_item_id") VALUES ($1,$2) ON CONFLICT ("ext_menu_id", "ext_menu_item_id") DO UPDATE SET "ext_menu_id" = EXCLUDED."ext_menu_id", "ext_menu_item_id" = EXCLUDED."ext_menu_item_id"'
    );
  });

  it('should build UPSERT with ignoreDuplicates as DO NOTHING', async () => {
    setColumnTypes({});
    const qb = new QueryBuilder(pool, client, 'ai_prompt_snapshots', 'public')
      .upsert(
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."ai_prompt_snapshots" ("prompt_hash","content") VALUES ($1,$2) ON CONFLICT ("prompt_hash") DO NOTHING'
    );
  });

  it('should keep native arrays without JSON stringifying', async () => {
    setColumnTypes({ tags: 'text[]', scores: 'int4[]' });
    const qb = new QueryBuilder(pool, client, 'native_array_test_table', 'public')
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."native_array_test_table" ("tags","scores") VALUES ($1,$2) RETURNING "tags", "scores"'
    );
  });

  it('should stringify jsonb and bigint on insert', async () => {
    (client as any).getColumnPgType = jest.fn(async (_schema: string, _table: string, column: string) => {
      if (column === 'bigint_value') return 'bigint';
      if (column === 'metadata') return 'jsonb';
      return 'text';
    });

    const metadata = { ok: true, id: 987n };
    const qb = new QueryBuilder(pool, client, 'sample', 'public')
    ...
    expect(normalizeSql(query)).toBe(
      'INSERT INTO "public"."sample" ("bigint_value","metadata","name") VALUES ($1,$2,$3) RETURNING *'
    );
  });
  ...
  it('should stringify jsonb and bigint on update', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'UPDATE "public"."sample" SET "bigint_value" = $1, "metadata" = $2, "name" = $3 WHERE "id" = $4 RETURNING *'
    );
  });
  ...
  it('should build UPDATE with IS NULL where clause', async () => {
    ...
    expect(values).toEqual(['active', 1]);
    expect(normalizeSql(query)).toBe(
      'UPDATE "public"."users" SET "status" = $1 WHERE "id" = $2 AND "deleted_at" IS NULL RETURNING *'
    );
  });
  ...
  it('should build DELETE with RETURNING when select() is called', async () => {
    ...
    expect(values).toEqual([1]);
    expect(normalizeSql(query)).toBe(
      'DELETE FROM "public"."posts" WHERE "user_id" = $1 RETURNING "id"'
    );
  });
  ...
  it('should quote reserved keywords in select/where/order', async () => {
    ...
    expect(normalizeSql(query)).toBe(
      'SELECT "id", "order", "desc", "user" FROM "public"."reserved_keyword_test_table" WHERE "order" = $1 ORDER BY "order" DESC'
    );
  });

  it('should build array-embed join subquery for 1:N relations', async () => {
    (client as any).getForeignKey = jest.fn(async (_schema: string, table: string, foreignTable: string) => {
    ...
    const qb = new QueryBuilder(pool, client, 'authors', 'public')
      .select('*, books(title)');
  ...
  it('should build object-embed join subquery for N:1 relations', async () => {
    ...
    const qb = new QueryBuilder(pool, client, 'books', 'public')
  ...
  it('should support inner embed with related table filters', async () => {
    ...
    const qb = new QueryBuilder(pool, client, 'cur_menu_item', 'public')
  ...
  it('should build nested embed queries with inner relations', async () => {
    ...
    const qb = new QueryBuilder(pool, client, 'menu_item_opts_set', 'public')
      .select(`
        id,
        created_at,
        menu_item_id,
