...
describe('manual transaction handle (begin/commit/rollback, scoped)', () => {
    ...
    beforeEach(async () => {
        ...
        db = new postgres_client_1.SupaLitePG({ pool }); // shared "singleton"
    });
    afterAll(async () => {
        await pool.query('DROP TABLE IF EXISTS manual_tx_test;');
        await pool.end();
    ...
    it('concurrent commit()+rollback() on one handle: one wins, the other throws, connection not double-released', async () => {
        const tightPool = new pg_1.Pool({ connectionString, max: 1 });
        try {
            const tight = new postgres_client_1.SupaLitePG({ pool: tightPool });
            ...
            expect(rejected[0].reason.message).toMatch('no active transaction');
    ...
    it('metadata cached inside a transaction does not pollute the shared singleton cache', async () => {
        ...
        try {
            ...
            expect(tx.schemaCache.has('public.manual_tx_test')).toBe(true);
            expect(db.schemaCache.has('public.manual_tx_test')).toBe(false);
        }
        finally {
            await tx.rollback();
        }
