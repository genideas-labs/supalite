...
const poolOf = (client) => client.pool;
describe('postgres-client (live DB coverage)', () => {
    let setupPool;
    beforeAll(async () => {
        setupPool = new pg_1.Pool({ connectionString });
    ...
    `);
        ...
        await setupPool.query(`INSERT INTO ${SCHEMA}.cov_books (id, title, big_col, author_id) VALUES (1, 'x', 9007199254740992, 1);`);
    });
    afterAll(async () => {
        await setupPool.query(`DROP SCHEMA IF EXISTS ${SCHEMA} CASCADE;`);
        await setupPool.end();
    ...
    describe('constructor', () => {
        test('logs and does not own an externally supplied pool (verbose)', () => {
            ...
            const client = new postgres_client_1.SupaLitePG({ pool, verbose: true });
        ...
        test('throws (and logs) on a malformed connection string', () => {
            ...
            expect(() => new postgres_client_1.SupaLitePG({ connectionString: 'mysql://nope/db' })).toThrow(/Failed to establish database connection/);
            expect(errSpy).toHaveBeenCalledWith('[SupaLite ERROR] Database connection error:', expect.stringContaining('Invalid PostgreSQL connection string format'));
            errSpy.mockRestore();
        });
        test('uses individual DB_* parameters and logs them masked when verbose', async () => {
            ...
            try {
                ...
                if (saved !== undefined)
                    process.env.DB_CONNECTION = saved;
            }
            const call = logSpy.mock.calls.find((c) => String(c[0]).includes('individual parameters'));
            expect(call).toBeDefined();
            // Password must be redacted in the log payload.
            expect(call[1].password).toBe('********');
            logSpy.mockRestore();
            await client.close();
        });
        test('an owned pool logs when it emits an idle-client error', () => {
            ...
            expect(errSpy).toHaveBeenCalledWith('[SupaLite ERROR] Unexpected error on idle client', expect.any(Error));
            errSpy.mockRestore();
            return client.close();
        });
    });
    ...
    describe('bigint transform parsers', () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const readBigint = async (client, literal) => {
            const r = await poolOf(client).query(`SELECT ${literal}::bigint AS v`);
            return r.rows[0].v;
        };
        test("'string' returns the raw string form", async () => {
            const client = new postgres_client_1.SupaLitePG({ connectionString, bigintTransform: 'string' });
        ...
        test("'number' coerces to Number, passes null through, and warns on unsafe values (verbose)", async () => {
            ...
            expect(nul).toBeNull();
            expect(warnSpy.mock.calls.some((c) => String(c[0]).includes('might lose precision'))).toBe(true);
            warnSpy.mockRestore();
            await client.close();
        });
        test("'number-or-string' keeps precision as string for unsafe, Number for safe (verbose warns)", async () => {
            ...
            expect(safe).toBe(42);
            expect(warnSpy.mock.calls.some((c) => String(c[0]).includes('exceeds safe integer range'))).toBe(true);
            warnSpy.mockRestore();
            await client.close();
        });
        test("'bigint' returns a native BigInt", async () => {
            const client = new postgres_client_1.SupaLitePG({ connectionString, bigintTransform: 'bigint' });
            const v = await readBigint(client, `'9007199254740992'`);
    ...
    describe('rpc', () => {
        test('calls a scalar function with named params and unwraps the scalar result', async () => {
            const client = new postgres_client_1.SupaLitePG({ connectionString, schema: SCHEMA });
        ...
        test('returns a PostgresError when the function does not exist (execute catch path)', async () => {
            ...
            expect(res.error).toBeInstanceOf(errors_1.PostgresError);
    ...
    describe('testConnection and close', () => {
        ...
        test('close() is a no-op for a non-owned (external) pool', async () => {
            const pool = new pg_1.Pool({ connectionString });
            const client = new postgres_client_1.SupaLitePG({ pool });
